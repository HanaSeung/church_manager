    ...
    const ROLE_GROUPS = {
      ...
      '선교사': ['선교사'],
      '사모': ['사모'],
      '없음': [''],   // 비성도·타종교 등 직분이 없는 사람. 세부는 빈칸 → 배지 미표시, 정렬 맨 뒤.
    };
    const ROLE_CATS = Object.keys(ROLE_GROUPS);
    const OFFICER_CATS = ['목사', '장로', '안수집사', '권사'];
    // 세부직분 → 대분류 역조회. 빈 값은 '없음'(직분 미부여)이 아니라 '미상'으로 보고 ''를 돌려준다.
    // ('없음' 대분류의 세부가 빈 문자열이라, 가드 없으면 옛 데이터의 빈 role이 전부 '없음'으로 잡힌다)
    function roleCatOf(sub) { if (!sub) return ''; return ROLE_CATS.find((c) => ROLE_GROUPS[c].includes(sub)) || ''; }
    ...
    // 나이 계산. refDate(별세일)가 있으면 그 날 기준으로 고정 → 별세자의 나이가 매년 늘지 않는다.
    function calcAge(birth, refDate) {
      const m = (birth || '').trim().match(/^(\d{4})[-.\/](\d{1,2})[-.\/](\d{1,2})$/);
      if (!m) return null;
      const by = +m[1], bm = +m[2], bd = +m[3];
      const r = (refDate || '').trim().match(/^(\d{4})-(\d{2})-(\d{2})$/);
      const t = r ? new Date(+r[1], +r[2] - 1, +r[3]) : new Date();
      let age = t.getFullYear() - by;
      if ((t.getMonth() + 1) < bm || ((t.getMonth() + 1) === bm && t.getDate() < bd)) age--;
      return (age >= 0 && age < 130) ? age : null;
    }
    // 나이 표기: 별세자는 '향년', 생존자는 '만'
    const ageLabel = (m) => (m && m.deceasedDate) ? '향년' : '만';
    const ageOf = (m) => (m ? calcAge(m.birth, m.deceasedDate) : null);
    ...
    function renderList() {
      const isDead = (m) => m.memberType === '별세';
      const na = allMembers.filter((m) => m.archived).length;
      const nd = allMembers.filter((m) => !m.archived && isDead(m)).length;
      const ta = $('ftabArchived');
      ta.style.display = na ? '' : 'none';
      ta.textContent = '보관됨 ' + na;
      const td = $('ftabDeceased');
      td.style.display = nd ? '' : 'none';
      td.textContent = '별세 ' + nd;
      if ((currentFilter === 'archived' && na === 0) || (currentFilter === 'deceased' && nd === 0)) {
        currentFilter = 'all';
        document.querySelectorAll('.ftab').forEach((x) => x.classList.toggle('ftab-on', x.dataset.filter === 'all'));
      }
      // 별세자는 '전체'를 포함한 일반 탭에서 제외한다(데이터는 남기고 화면에서만 뺀다).
      // 배우자의 사별일·헌금 기록·가정 구성원 연결이 유지되어야 하므로 archived(숨김) 처리하지 않는다.
      const live = allMembers.filter((m) => !m.archived && !isDead(m));
      ...
      else if (currentFilter === 'deceased') rows = allMembers.filter((m) => !m.archived && isDead(m));
      else {
        rows = allMembers.filter((m) => !m.archived && !isDead(m));
    ...
    function dIdentity(m) {
      const age = ageOf(m);
      const top = [m.gender, (age != null ? `${ageLabel(m)} ${age}세` : null), m.memberType, m.status].filter(Boolean).join(' · ');
    ...
    function dIdentityGreen(m) {
      const age = ageOf(m);
      const meta = [m.role, m.gender, (age != null ? `${ageLabel(m)} ${age}세` : null), m.status].filter(Boolean).join(' · ');
    ...
    // 사별일 = 배우자(별세자)의 별세일. 죽은 쪽에만 저장하고 산 쪽은 끌어다 쓴다.
    function spouseDeathDate(m) {
      if (!m || !m.spouseId) return '';
      const sp = allMembers.find((x) => x.id === m.spouseId);
      return (sp && sp.deceasedDate) ? sp.deceasedDate : '';
    }
    function marriageTxt(m) {
      if (!m || !m.marriage) return '';
      if (m.marriage === '사별') {
        const d = spouseDeathDate(m);
        return d ? `사별 · ${dot(d)}` : '사별';
      }
      return m.marriage;
    }
    function dFields(m) {
      const age = ageOf(m);
      const birthTxt = m.birth ? (m.birth + (m.birthCal === 'lunar' ? ' (음)' : (m.birthCal === 'solar' ? ' (양)' : '')) + (age != null ? ` · ${ageLabel(m)} ${age}세` : '')) : '';
      ...
      const wed = [marriageTxt(m), dot(m.wedDate)].filter(Boolean).join(' · ');
      const core = [
      ...
      const extra = [
        ...
        ['church', '출석교회', m.attendChurch],
        ['calendar', '별세일', dot(m.deceasedDate)],
      ];
      return { core, extra };
    }
    ...
    function dFamilyCard(m) {
      ...
      return `<div style="font-size:12px; color:var(--muted); margin:14px 4px 6px;">가정 구성원</div>
        <div style="background:var(--card); border:1px solid var(--line); border-radius:12px; overflow:hidden;">${fam.map((x, i) => {
          ...
          const kage = ageOf(x); const relTxt = relD + ((kage != null && /아들|딸|손자|손녀/.test(relD)) ? ` · ${kage}세` : '');
    ...
    function dColsBody(m) {
      const age = ageOf(m);
      const birthTxt = m.birth ? (esc(m.birth) + (m.birthCal === 'lunar' ? ' (음)' : (m.birthCal === 'solar' ? ' (양)' : '')) + (age != null ? ` · ${ageLabel(m)} ${age}세` : '')) : '';
      return `<div class="dsec">기본</div>
         ...
         ${drow('출석교회', esc(m.attendChurch))}
         ${drow('별세일', esc(dot(m.deceasedDate)))}
         <div class="dsec">가족</div>
         ${drow('결혼관계', esc(marriageTxt(m)))}
    ...
    function openEdit(id) {
      ...
      // 신규 등록일 때만 오늘 날짜를 기본값으로. 기존 성도는 빈 값을 그대로 유지한다.
      // (m?.regDate || today) 로 하면 등록일을 지운 성도가 다시 열 때 오늘 날짜로 되살아난다.
      $('fRegDate').value = m ? (m.regDate || '') : new Date().toISOString().slice(0, 10);
      ...
      $('fDeceased').value = m?.deceasedDate || '';
      $('fAttendChurch').value = m?.attendChurch || '';
      syncDeceasedRow();
    ...
    function updateAge() {
      // 교인구분='별세'면 별세일 기준으로 고정(향년). 아니면 오늘 기준(만).
      const dead = ($('fType').value === '별세') ? $('fDeceased').value : '';
      const a = calcAge($('fBirth').value, dead);
      $('ageBox').textContent = (a != null) ? `${dead ? '향년' : '만'} ${a}세` : '–';
    }
    ...
    function normDateStr(v, precision) {
      const digits = (v || '').replace(/\D/g, '');
      if (digits === '') return { val: '', ok: true };
      // 'partial': 연(4) / 연월(6) / 연월일(8) 모두 허용. 아는 만큼만 입력.
      if (precision === 'partial') {
        if (digits.length === 4) return { val: digits, ok: true };
        if (digits.length === 6) {
    ...
    function setupDateInput(id, errId, calId, pickId, precision) {
      ...
      const re = precision === 'month' ? /^\d{4}-\d{2}$/
        : precision === 'partial' ? /^\d{4}-\d{2}-\d{2}$/   // 달력엔 완전한 날짜만 넘긴다
        : /^\d{4}-\d{2}-\d{2}$/;
      function apply() {
    ...
    // 신급일·결혼일: 연(1985) / 연월(1985-03) / 연월일(1985-03-15) 모두 허용 — 아는 만큼만 입력
    const applyGradeDate = setupDateInput('fGradeDate', 'eGradeDateErr', 'fGradeDateCal', 'fGradeDatePick', 'partial');
    const applyWedDate = setupDateInput('fWed', 'eWedErr', 'fWedCal', 'fWedPick', 'partial');
    const applyPrayDate = setupDateInput('fPrayDate', 'ePrayDateErr', 'prayDateCal', 'prayDatePick', 'date');
    const applyDeceased = setupDateInput('fDeceased', 'eDeceasedErr', 'fDeceasedCal', 'fDeceasedPick', 'date');

    // ── 교인구분에 따른 조건부 칸 (별세일 / 출석교회) ──
    function syncDeceasedRow() {
      const t = $('fType').value;
      const dead = (t === '별세');
      const other = (t === '타교인');
      $('rowDeceased').style.display = dead ? '' : 'none';
      $('rowAttendChurch').style.display = other ? '' : 'none';
      if (!dead) { $('fDeceased').value = ''; $('fDeceased').classList.remove('invalid'); $('eDeceasedErr').classList.remove('show'); }
      if (!other) { $('fAttendChurch').value = ''; }
      updateAge();   // 별세 여부에 따라 만/향년 전환
    }
    $('fType').addEventListener('change', syncDeceasedRow);
    $('fDeceased').addEventListener('input', updateAge);
    $('fDeceased').addEventListener('blur', updateAge);
    $('fDeceasedPick').addEventListener('change', updateAge);
    ...
    $('dupBtn').addEventListener('click', handleDupCheck);
    ...
    // Enter = 옆 버튼 클릭 (이름→동명이인 확인, 세대주/배우자/인도자→명부 검색)
    const onEnter = (id, fn) => $(id).addEventListener('keydown', (e) => {
      if (e.key !== 'Enter') return;
      e.preventDefault();
      fn();
    });
    onEnter('fName', handleDupCheck);
    onEnter('fHead', handleHeadSearch);
    onEnter('fSpouse', handleSpouseSearch);
    onEnter('fGuide', handleGuideSearch);
    ...
    async function saveMember() {
      ...
      const base = {
        ...
        deceasedDate: $('fType').value === '별세' ? $('fDeceased').value : '',
        attendChurch: $('fType').value === '타교인' ? $('fAttendChurch').value.trim() : '',
