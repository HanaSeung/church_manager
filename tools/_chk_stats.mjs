  ...
  async function loadNodes() {
    ...
    if (curFY == null) {
      ...
      curFY = ys.length ? Math.max(...ys) : fiscalYearOf(new Date());
    }
  }
  ...
  $('fySel').onchange = async () => { curFY = Number($('fySel').value); await reload(); };

  async function reload() {
    const r = fyRange(curFY);
    $('rangeTxt').textContent = `${r.from} ~ ${r.to}`;
    ...
    try {
      ...
      incDocs = a.docs.map((d) => d.data());
      expDocs = b.docs.map((d) => d.data());
      agg = aggregate();
      renderAll();
  ...
  function aggregate() {
    ...
    const build = (kind, docs) => {
      ...
      const keyMap = {};                       // catKey → node
      list.forEach((n) => { keyMap[catKey(pathOf(n, byId))] = n; });

      const actual = {};                       // nodeId → 결산 합계
      const months = {};                       // nodeId → [12개월]
      let unmatchedCnt = 0, unmatchedAmt = 0;  // 항목을 못 찾은 기록
      let nonLeafCnt = 0, nonLeafAmt = 0;      // 하위항목이 생겨 표에 자리가 없는 기록
      docs.forEach((d) => {
        const amt = Number(d.amount || 0);
        const p = d.catPath || [d.c1, d.c2, d.c3].filter(Boolean).join(' › ');
        const n = keyMap[catKey(p)];
        ...
        const mi = monthIdx(curFY, d.date);
        if (mi >= 0) {
          if (!months[n.id]) months[n.id] = new Array(12).fill(0);
          months[n.id][mi] += amt;
        }
      });
      ...
      const rows = leaves.map((n) => {
        ...
        return {
          ...
          months: months[n.id] || new Array(12).fill(0)
        };
      });
  ...
  function renderAll() {
    ...
    html.push(monthlyHtml('수입내역 (월별)', agg.normal.concat(agg.special)));
    html.push(monthlyHtml('지출내역 (월별)', agg.expRows));
  ...
  function tableHtml(title, sub, groups, totLabel) {
    ...
    return `<div class="sect">
      ...
      <table class="t">
        ...
        <thead><tr><th>항목</th><th>소항목</th><th>예산</th><th>결산</th><th>차액</th></tr></thead>
  ...
  function balanceHtml() {
    ...
    return `<div class="sect">
      ...
      <table class="t" style="margin-top:8px;">
        ...
        <thead><tr><th>기금</th><th>이월금</th><th>수입</th><th>지출</th><th>잔액</th></tr></thead>
        <tbody>${rows}
          <tr class="tot">
  ...
  // ---- 월별 내역 (12개월 가로표) ----
  // 이월은 넣지 않는다. 수입이 아니고, 12월 한 칸에 뭉쳐 표를 왜곡한다.
  function monthlyHtml(title, rows) {
    if (!rows.length) return '';
    const labels = monthLabels(curFY);
    const groups = groupByC1(rows);
    ...
    groups.forEach((g) => {
      ...
      g.rows.forEach((r, i) => {
        r.months.forEach((v, k) => { gm[k] += v; totM[k] += v; });
        body.push(line('', i === 0 ? g.c1 : '', r.name, r.months, r.actual));
      });
      if (groups.length > 1) body.push(line('sub', '', '소계', gm, g.actual));
    });
    body.push(line('tot', '', '합계', totM, sumA(rows)));
  ...
  function sheetMonthly(kind, rows, title) {
    const labels = monthLabels(curFY);
    const groups = groupByC1(rows);
  ...
  $('xlsBtn').onclick = async () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb,
        sheetMonthly('income', agg.normal.concat(agg.special), `${curFY}년 수입내역(${r.from.slice(0, 7).replace('-', '.')}~${r.to.slice(0, 7).replace('-', '.')})`),
        '수입내역');
      XLSX.utils.book_append_sheet(wb,
        sheetMonthly('expense', agg.expRows, `${curFY}년 지출내역(${r.from.slice(0, 7).replace('-', '.')}~${r.to.slice(0, 7).replace('-', '.')})`),
        '지출내역');
