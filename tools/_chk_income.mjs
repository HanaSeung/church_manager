    ...
    const DEFAULT_INCOME = [
      ...
      ['100220', '이월', 1],
      ['100230', '일반재정 이월', 2],
      ['100240', '선교헌금 이월', 2], ['100250', '장학헌금 이월', 2],
      ['100260', '건축헌금 이월', 2], ['100270', '차량헌금 이월', 2]
    ];
    ...
    function openModal(title, parentText) {
      ...
      clearErr();
      if (!editId) { $('mCode').value = nextCode(); $('mName').value = ''; }
    ...
    async function save() {
      ...
      try {
        if (editId) {
          await updateDoc(doc(db, COL, editId), { code, name, updatedAt: serverTimestamp() });
        } else {
