  ...
  // 연동 성도의 등록 배우자 이름을 배우자칸에 채움(배우자 미등록이면 비움 → 직접 입력)
  function fillSpouseName(mem) {
    const sp = mem && mem.spouseId ? members.find((x) => x.id === mem.spouseId) : null;
  ...
  function setLinked(m) {
    ...
    if ($('spouseChk').checked) fillSpouseName(m);
  }
  function onNameEdit() {
  ...
  function toggleSpouse() {
    ...
    if (!on) { $('spouseName').value = ''; return; }
    // 체크하는 즉시: 이미 연동된 성도가 있으면 그 배우자 이름을 자동입력(재검색 불필요)
    if (linked) fillSpouseName(members.find((x) => x.id === linked.id));
    $('spouseName').focus();
  }
  // ----- 저장 -----
  ...
  async function save() {
    ...
    try {
      ...
      const keepSpouse = curTab === 'inc' && $('spouseChk').checked;   // 2-A: 저장 후 배우자 체크 상태만 유지(이름은 초기화)
      resetForm();
      if (keepSpouse) { $('spouseChk').checked = true; $('spouseName').disabled = false; }
      await loadList();
      return true;
  ...
  // 날짜순 뷰 필터: 입력 폼의 '헌금 항목' 선택으로 조회기간 내 기록을 거른다.
  // (항목별 뷰는 전체 유지. 매칭은 저장 문서의 code → 이름경로(catPath) 순으로 판별)
  function incDateFilter(rows) {
    const it = config.income[+$('incCat').value];
    if (!it) return rows;
    const code = it.code || '', want = catKey(pathOf(it));
    return rows.filter((r) => (code && (r.code || '') === code) ||
      catKey(r.catPath || [r.c1, r.c2, r.c3].filter(Boolean).join(' › ')) === want);
  }
  function renderIncList(rows) {
    const box = $('listRows');
    if (incView !== 'group') rows = incDateFilter(rows);
    // 합계는 화면에 보이는 행 기준 (날짜순=선택 항목만, 항목별=전체)
    const sum = rows.reduce((s, r) => s + (Number(r.amount) || 0), 0);
    $('listSum').textContent = `${rows.length}건 · ${wonFmt(sum)}원`;
    if (!rows.length) { box.innerHTML = '<div class="empty">이 기간에 선택한 항목의 기록이 없습니다.</div>'; return; }
  ...
  async function initUI() {
    ...
    $('inDate').addEventListener('change', () => { $('weekBadge').textContent = $('inDate').value ? weekLabel($('inDate').value) : '–'; refreshIncomeCats(); refreshExpenseCats(); if (curTab === 'inc' && incView !== 'group' && lastIncRows.length) renderIncList(lastIncRows); });
    $('incCat').addEventListener('change', () => { setCatMiss(false); updateCode(); updateIncFace(); if (curTab === 'inc' && incView !== 'group') renderIncList(lastIncRows); });
    ...
    // PC(데스크톱): inputmode="numeric"가 크롬에서 IME를 영문으로 꺼, 금액칸을 거친 뒤 이름칸 한글 입력이 깨짐 → 데스크톱에선 제거(모바일 숫자패드는 유지)
    if (window.matchMedia('(hover: hover) and (pointer: fine)').matches) $('inAmount').removeAttribute('inputmode');
