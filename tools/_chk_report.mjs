  ...
  // 년간 월별 회기 비교: 표를 세로로 쌓는다. 각 원소가 표 하나의 회기(연도)다.
  //   구분·특별헌금·기타(curKind/showSp/showEtc)는 전역이라 모든 표에 함께 걸린다. 회기만 표마다 다르다.
  let monFys = [];                  // monthly: 비교할 회기 목록 (첫 진입은 [curFY])
  let docsByFy = {};                // monthly: 회기별 { inc, exp } 로컬 조회 결과 (읽기 0)
  ...
  function buildControls() {
    ...
    // 년간 월별: 회기를 표마다 각자 고른다 → 상단바 회기 select 는 숨기고 '＋ 표 추가'를 낸다.
    if (TYPE === 'monthly') {
      if (!monFys.length) monFys = [curFY];
      $('fySel').hidden = true;
      $('addFyBtn').hidden = false;
      $('addFyBtn').onclick = () => { monFys.push(monFys[0] != null ? monFys[0] : curFY); reload(); };
    }
    syncControls();
  }
  ...
  // ---- 년간 월별 회기 비교: 표별 회기 변경 · 삭제 위임 ----
  // 표는 renderAll 마다 새로 그려지므로 #body 에 한 번만 위임한다.
  $('body').addEventListener('change', (e) => {
    const s = e.target.closest('select[data-fy]');
    if (!s) return;
    monFys[Number(s.dataset.fy)] = Number(s.value);
    reload();
  });
  $('body').addEventListener('click', (e) => {
    const del = e.target.closest('[data-delfy]');
    if (!del) return;
    monFys.splice(Number(del.dataset.delfy), 1);
    reload();
  });
  ...
  async function reload() {
    ...
    // 년간 월별 회기 비교: 표마다 회기가 다르다. 회기별로 로컬에서 읽어 docsByFy 에 모은다.
    //   전부 로컬(IndexedDB)이라 회기가 몇 개든 Firestore 읽기 0. 구분 전환도 재조회 없이 다시 그린다.
    if (TYPE === 'monthly') {
      $('rangeTxt').textContent = '';
      $('body').innerHTML = '<div class="loading">불러오는 중…</div>';
      try {
        docsByFy = {};
        await Promise.all([...new Set(monFys)].map(async (fy) => {
          const rr = fyRange(fy);
          const [ic, ex] = await Promise.all([
            localRange('offerings', rr.from, rr.to),
            localRange('expenses', rr.from, rr.to)
          ]);
          docsByFy[fy] = { inc: ic, exp: ex };
        }));
        // renderWarn 등이 전역 agg 를 참조하므로, 렌더 전에 대표 회기(monFys[0])로 미리 맞춰 둔다.
        const rep = monFys[0];
        if (rep != null && docsByFy[rep]) {
          curFY = rep; setBucket();
          incDocs = docsByFy[rep].inc; expDocs = docsByFy[rep].exp;
          agg = aggregate();
        }
        renderAll();
      } catch (e) {
  ...
  function renderAll() {
    ...
    if (TYPE === 'bs') {
      ...
      // 회기 비교(B안): 전역(curFY·BK·incDocs·expDocs·agg)을 회기마다 잠깐 갈아끼우며 표를 하나씩 뽑는다.
      //   회기는 표마다 다르지만 구분·특별헌금·기타는 전역이라 모든 표에 똑같이 걸린다.
      //   합계·소계 규칙은 단일 표와 같다(기타 체크 시에만 합계). 끝나면 대표 회기로 반드시 원복한다.
      const keepFy = curFY;
      const title = (curKind === 'income') ? '수입내역 (월별)' : '지출내역 (월별)';
      monFys.forEach((fy, idx) => {
        curFY = fy; setBucket();
        const d = docsByFy[fy] || { inc: [], exp: [] };
        incDocs = d.inc; expDocs = d.exp; agg = aggregate();
        const head = fyHeadCtrl(fy, idx);
        html.push(curKind === 'income'
          ? incGrid(title, head, true)
          : gridHtml(title, agg.expRows, null, { fixed: true }, head));
      });
      // 원복: 이후 상단바 조작이 대표 회기 기준으로 돌게 전역을 되돌린다.
      curFY = keepFy; setBucket();
      const d0 = docsByFy[keepFy] || { inc: [], exp: [] };
      incDocs = d0.inc; expDocs = d0.exp; agg = aggregate();
    } else if (TYPE === 'weekly') {
  ...
  function renderWarn() {
    const box = $('warnBox');
    // 년간 월별 첫 렌더 등 agg 가 아직 없을 때 참조하면 터진다 → 경고를 숨기고 조용히 빠진다.
    if (!agg) { box.classList.remove('on'); box.innerHTML = ''; return; }
    const w = [];
    const add = (label, o) => {
  ...
  // 년간 월별 회기 비교: 표 제목줄 우측의 회기 select + 삭제(×) 버튼 HTML.
  //   표가 2개 이상일 때만 ×를 낸다 — 최소 한 개는 남긴다.
  function fyHeadCtrl(fy, idx) {
    const ys = [...new Set(nodes.map((c) => Number(c.fy)).filter(Boolean))];
    if (!ys.includes(fy)) ys.push(fy);
    ys.sort((a, b) => b - a);
    const opts = ys.map((y) => `<option value="${y}"${y === fy ? ' selected' : ''}>${y}년도</option>`).join('');
    const del = (monFys.length > 1)
      ? `<button class="delx" data-delfy="${idx}" title="이 표 삭제" aria-label="이 표 삭제">✕</button>` : '';
    return `<span class="rt"><select class="fyin" data-fy="${idx}">${opts}</select>${del}</span>`;
  }

  // 수입 월별/주별 그리드. 상단바 '기타' 체크에 따라 '기타' 대분류를 넣거나 뺀다.
  //   끔(기본): 일반재정만 + 소계(합계 줄 없음 — 소계가 곧 총액) / 켬: 일반재정+기타 + 합계.
  //   headCtrl 은 제목줄 우측 컨트롤. fixed 는 회기 비교의 고정 격자 여부.
  function incGrid(title, headCtrl, fixed) {
    const tail = showSp ? agg.special : [];
    if (showEtc) return gridHtml(title, agg.normal, tail, { fixed }, headCtrl);
    const gen = agg.normal.filter((r) => r.c1 !== '기타');
    return gridHtml(title, gen, tail, { forceSub: true, noTotal: true, fixed }, headCtrl);
  }
  ...
  //   헌금 › 기타 › [합계 = 헌금+기타] › 특별헌금
  function gridHtml(title, rows, tailRows, opts, headCtrl) {
    ...
    // 회기 비교(fixed): 모든 표가 같은 col 폭을 써야 세로로 정렬된다.
    //   폭은 최대 금액(억대 합계)이 안 잘리게 넉넉히 — 월 84px, 합계 98px.
    const colg = opts.fixed
      ? `<colgroup><col style="width:66px"><col style="width:88px">${'<col style="width:84px">'.repeat(BK.n)}<col style="width:98px"></colgroup>`
      : '';
    return `<div class="sect">
      <div class="sh">${esc(title)}<span class="sub">가로로 밀어서 보세요</span>${headCtrl || ''}</div>
      <div class="scrollx">
        <table class="m${opts.fixed ? ' fixw' : ''}">
          ${colg}
          <thead><tr>
